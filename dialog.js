...
(function($) {
    ...
    var Utils = {
        ...
        },//获取到id=mask的项，并且将该块插入到body的前面位置；
        setPosition: function(elem, options) {//设置弹出框的位置，用extend原型函数；
            ...
            if (null !== opts.target) {
                ...
                var cwidth = document.documentElement.clientWidth,//客户端
    ...
    $.extend(Dialog.prototype, {
        ...
        _buttonCreate : function(){
            ...
            if(self.options.hasButtons){
                ...
                $.each(self.options.buttons, function(name, fn){
                    ...
                    btnObj.type = 1;
                    if(name == "取消" || name=="关闭" || name=="返回首页"){// dongzhi 为status sim卡变更时的t关闭渲染成非引导型在此加入
                        btnObj.type = 2;
                        if(!fn){
                            cancelKey = true;
                        }
        ...
        _instanceEvent : function(){
            ...
            self.dialogDom.addClass('animated jello');
        ...
        _close : function(){
            ...
            // self.dialogDom.remove();
            self.dialogDom.addClass('animated rotateOutDownLeft');
    ...
    var dialog_tmpl = '' +
        '<div id = "${id}" class = "dialog ${dialogClass}" style="z-index:${zIndex}">' +
            '<div class="dialog-td">' +
                ...
                '</div>' +
                '{@if topRemaind}' +
                    '<div class="dialog-remind dialog-remind-height${topRemaindHeightNumber}">'+
                        '<i class="dialog-remind-l"></i>'+
                        '<i class="dialog-remind-r"></i>'+
                        '<div class="dialog-remind-txt">$${topRemaindContainer}</div>'+
                    '</div>'+
                '{@/if}' +
                ...
                '<div class = "dialog-buttonpane">' +
                    '{@if btnHasDashed == true}<div class="dialog-buttonpane-dashed"></div>{@/if}' +
                    '<div class = "dialog-button-container button-container ${btnClassName}">' +
    ...
    var btn_tmpl = '<a href = "{@if href}${href}{@else}javascript:;{@/if}"' +
        ' {@if id}id = ${id}{@/if} class = "newbtn newbtn-${type}">' +
            '${text}' +
        '</a>';
